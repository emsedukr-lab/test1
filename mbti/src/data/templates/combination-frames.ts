...
export const COMBINATION_FRAMES: Record<CombinationKind, readonly CombinationRenderer[]> = {
  "major-dominant": [
    ...
    (c) =>
      `배열의 무게중심이 메이저 카드 쪽에 실려 있습니다. ${c.topicNameKo}의 이번 국면에서는 잔가지보다 뿌리를 먼저 살펴볼 만합니다.`,
  ],
  "major-absent": [
    (c) =>
    ...
    (c) =>
      `이번에는 마이너 카드들만 테이블에 올라왔습니다. ${josa(c.contextNoun, "은/는")} 지금 손닿는 범위 안에서 조율할 수 있는 문제로 읽힙니다.`,
  ],
  "suit-dominant": [
    (c) =>
    ...
    (c) =>
      `배열의 색이 ${c.suitNameKo} 하나로 짙게 물들어 있습니다. 그만큼 ${c.topicNameKo}에서 이 에너지가 차지하는 비중이 크다는 뜻일 수 있습니다.`,
  ],
  "topic-suit-missing": [
    (c) =>
    ...
    (c) =>
      `${c.suitNameKo} 없이 펼쳐진 ${c.topicNameKo} 배열입니다. 익숙한 접근 대신 다른 결의 힘을 빌려보라는 힌트로 보입니다.`,
  ],
  "repeated-number": [
    (c) =>
    ...
    (c) =>
      `숫자 ${c.number} 카드가 배열 곳곳에 자리를 잡았습니다. 지금의 ${josa(c.contextNoun, "이/가")} 하나의 공통된 국면 위에 서 있다는 표시일 수 있습니다.`,
  ],
  "court-heavy": [
    (c) =>
    ...
    (c) =>
      `배열에 인물의 얼굴이 유난히 많이 보입니다. ${c.topicNameKo}의 답을 상황이 아니라 관계의 자리에서 찾아볼 만합니다.`,
  ],
  "ace-cluster": [
    () =>
    ...
    (c) =>
      `배열 위에 에이스의 기운이 나란히 켜졌습니다. 모든 문을 동시에 열기보다, 지금의 ${josa(c.contextNoun, "과/와")} 가장 가까운 문 하나부터 여는 것도 방법입니다.`,
  ],
  "expansion-flow": [
    () =>
    ...
    () =>
      `숫자의 걸음이 점점 커지는 배열입니다. 판이 넓어지는 시기에는 속도만큼 방향을 자주 확인하는 편이 도움이 될 수 있습니다.`,
  ],
  "convergence-flow": [
    () =>
    ...
    (c) =>
      `숫자의 걸음이 점점 안쪽으로 모이는 배열입니다. ${c.topicNameKo}에서 무엇을 남기고 무엇을 내려놓을지 고르는 국면일 수 있습니다.`,
  ],
};
